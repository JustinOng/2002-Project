...
public class AppUI extends MultiWindowTextGUI {
	...
	@Override
	/**
	 * Adds an additional window to the user interface, such as for a prompt,
	 * and waits for the user to handle or acknowledge the prompt before closing. 
	 */
	public WindowBasedTextGUI addWindowAndWait(Window window) {
		...
		if (getActiveWindow() != null) {
			/*
			 * In our use case, since we only display one window at a time, we shoudn't have an active 
			 *  window at this point because 'addWindowAndWait' should only return once the window closes.
			 * 
			 * This handles the case where the application window has been closed (eg by X),
			 * 'addWindowAndWait' returns immediately and there is still an active window. We throw 
			 * an exception which will bubble up to the main thread to terminate the application.
			 */
